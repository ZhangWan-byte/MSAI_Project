...
if os.path.exists(SIDE_CHAIN_FILE):
    ...
    print(f'Side chain coordination file not found at {SIDE_CHAIN_FILE}')
        ...
        for name, chain in zip(antigen_names + antibody_names, antigen_chains + antibody_chains):
            ...
            for i in range(len(chain)):
                try:
                    ...
                    print(f'{self.pdb_id}, chain {chain.get_id()}, residue at {chain.get_residue(i).get_id()} has no ca coordination, fill with other atoms')
